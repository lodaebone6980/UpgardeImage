...
# ==========================================
# [최종 수정됨] 함수: 프롬프트 생성 (Gems v4.1 기반 업그레이드)
# ==========================================
def generate_prompt(api_key, index, text_chunk, style_instruction, video_title, genre_mode="info", target_language="Korean"):
    ...
    # 고정 스타일 문장 (Gems v4.1)
    fixed_style_suffix = "The style is white circle-faced stickman 2D animation with simple lines and vivid color planes, emphasizing architectural atmosphere and emotional storytelling. All background signs, walls, and surfaces must have abstract geometric patterns without any text or writing."

    # ---------------------------------------------------------
    # [모드 1] 밝은 정보/이슈 (Gems v4.1 업그레이드)
    # ---------------------------------------------------------
    if genre_mode == "info":
        full_instruction = f"""
[역할]
당신은 2D 스틱맨 애니메이션 전문 프롬프트 디렉터입니다. 복잡한 상황을 직관적인 스틱맨 일러스트로 표현합니다.
...
[프롬프트 작성 공식 - 반드시 이 순서로 작성]
1단계 카메라와 캐릭터 개성: 카메라 앵글(로우앵글, 미디엄숏, 버드아이뷰 등)을 먼저 명시하고, 스틱맨에게 직업별 의상과 선명한 컬러를 부여하라.
   - 의상 예시: 형광 주황색 안전 조끼와 노란 헬멧을 쓴 건설 노동자, 보라색 턱시도에 빨간 나비넥타이를 맨 CEO, 에메랄드색 가운을 입은 의사, 핑크색 앞치마를 두른 요리사 등

2단계 다채로운 배경과 조명: 장소를 다양한 색감으로 묘사하라. 파란색만 쓰지 말고 에메랄드, 보라, 주황, 핑크, 민트, 골든앰버 등 화려한 색상을 사용하라.
   - 조명 예시: Golden Amber(따뜻함), Neon Pink(화려함), Mint Green(신비함), Sunset Orange(극적임)

3단계 글자 없는 배경 강제: 배경의 모든 간판과 벽면은 글자가 없는 추상적인 기하학적 패턴으로 표현하라. all background signs and walls are abstract geometric patterns without writing.

4단계 감정 표현: 스틱맨의 몸짓과 자세로 희로애락을 강렬하게 연출하라.

[필수 연출 지침]
1. 조명: 밝고 화사한 High Key Lighting을 사용하라. 그림자가 짙거나 어두운 부분 없이 산뜻하게.
2. 색감: 채도가 높고 선명한 다양한 색상을 사용하라. 칙칙하거나 회색조 금지.
3. 구성: 피사체를 화면 중앙에 명확하게 배치하라. 분할화면 금지, 하나의 장면으로 연출.
4. 분위기: 교육적이고 중립적이며 산뜻한 분위기. 우울하거나 무섭거나 기괴한 느낌 절대 금지.
5. 텍스트: {lang_guide} {lang_example}
6. 텍스트 배치: 화면 네 모서리나 가장자리에 글자 배치 금지. 중앙 피사체 주변에만 연출.

[임무]
제공된 대본 조각을 바탕으로 이미지 생성 AI가 그릴 수 있는 구체적인 묘사 프롬프트를 작성하라.

[작성 요구사항]
- 분량: 최소 5문장 이상 상세하게 묘사
- 포함 요소:
  - 카메라 앵글 명시
  - 스틱맨의 직업별 의상과 컬러
  - 구체적인 행동과 감정 표현
  - 다채로운 색감의 배경 묘사
  - 시각적 은유 (추상적 내용일 경우)
- 마지막에 반드시 이 문장 추가: {fixed_style_suffix}

[출력 형식]
- 순수 텍스트만 출력하라. 마크다운 강조 표시(**) 사용 금지.
- 한국어로만 작성하되, 마지막 고정 문장은 영어로 유지.
- 부가적인 설명 없이 오직 프롬프트 텍스트만 출력.
        """

    # ---------------------------------------------------------
    # [모드 2] 역사/다큐 (Gems v4.1 업그레이드)
    # ---------------------------------------------------------
    else: # genre_mode == "history"
        full_instruction = f"""
[역할]
당신은 2D 스틱맨 애니메이션 전문 프롬프트 디렉터이자 시대극 감독입니다. 역사적 순간을 스틱맨 일러스트로 표현합니다.
...
[프롬프트 작성 공식 - 반드시 이 순서로 작성]
1단계 카메라와 캐릭터 개성: 카메라 앵글을 먼저 명시하고, 스틱맨에게 시대와 역할에 맞는 의상과 컬러를 부여하라.
   - 의상 예시: 금색 테두리의 붉은 망토를 두른 로마 장군, 검은 갑옷에 보라색 깃털 장식을 단 기사, 에메랄드색 비단 한복을 입은 왕비, 주황색 승복을 입은 승려 등

2단계 시대적 배경과 조명: 시대와 장소의 특징을 다채로운 색감으로 묘사하라.
   - 조명 예시: Candlelight Amber(중세), Torch Orange(고대), Moonlight Silver(야간), Dawn Pink(새벽)
   - 배경 예시: 중세 유럽의 성, 산업혁명 런던 공장, 고대 로마 원형극장, 조선 궁궐 등

3단계 글자 없는 배경 강제: 배경의 모든 간판, 현수막, 벽면은 글자가 없는 추상적인 기하학적 패턴이나 문양으로 표현하라.

4단계 감정과 은유: 스틱맨의 몸짓으로 감정을 표현하고, 잔인한 장면은 시각적 은유로 대체하라.
   - 은유 예시: 전쟁/처형 -> 떨어진 붉은 장미, 붉게 물든 깃발, 부러진 칼
   - 은유 예시: 전염병/죽음 -> 검은 까마귀 떼, 시든 나무, 꺼진 촛불

[필수 연출 지침]
1. 매체: 무조건 평면적인 2D 일러스트레이션 또는 셀 애니메이션 스타일. 3D, 실사, 모델링 느낌 절대 금지.
2. 조명: 시네마틱 조명으로 극적인 분위기 연출. 시대극 특유의 무겁고 웅장한 톤.
3. 색감: 깊이 있고 진한 색조(Rich and Deep Tones). 다양한 색상 팔레트 사용.
4. 구성: 핵심 피사체를 화면 중앙에 배치. 분할화면 금지.
5. 텍스트: {lang_guide} {lang_example}
6. 텍스트 배치: 화면 네 모서리나 가장자리에 글자 배치 절대 금지.
